...
import { SHARE_HEADLINE, SHARE_SUB, buildShareCaption } from "@/mock/content";
import { captureShareCard, shareCardImage, shareResultMessage, type ShareChannel } from "@/lib/shareExport";

/**
 * SC — Share. Big number = week avg SFI (from /history trend); 7 bars from the
 * last 7 trend points; trend pill from weekly delta. Share buttons export the
 * card image plus caption (native share or download).
 */
export function ShareScreen() {
  const { history, scan, sfx } = useHlhp();
  ...
  const city = scan?.city ?? "My city";
  const caption = buildShareCaption(weekAvg, delta, city);
  ...
  async function onShare(kind: ShareChannel) {
    if (!cardRef.current || sharing) return;

    setSharing(true);
    try {
      const blob = await captureShareCard(cardRef.current);
      const result = await shareCardImage(blob, caption, kind);

      sfx("celebrate");
      setConfettiKey((k) => k + 1);
      setToast(shareResultMessage(result, kind));
    } catch {
      setToast("Could not export image — try again");
    } finally {
      setSharing(false);
  ...
  return (
    <Screen noPad className="bg-primary">
      <div className="relative aspect-[9/16] max-h-[560px] overflow-hidden" key={rk}>
        <SparkleLayer count={14} />
        <Confetti runKey={confettiKey} count={36} />

        <div
          ref={cardRef}
          className="relative h-full p-5 text-white"
          style={{ background: "linear-gradient(160deg, var(--accent-primary-dark) 0%, var(--primary) 55%, color-mix(in oklch, var(--primary) 80%, black) 100%)" }}
        >
        ...
        {toast && (
          <div className="pointer-events-none absolute bottom-12 left-1/2 z-20 -translate-x-1/2 rounded-full bg-white/95 px-4 py-1.5 text-[11px] font-medium text-primary shadow-lg">
            {toast}
          </div>
        )}
      </div>
      ...
      <div className="mx-3.5 mb-3.5 rounded-xl border border-border bg-card p-3">
        ...
        <div className="whitespace-pre-wrap text-[11.5px] leading-relaxed text-secondary-foreground">{caption}</div>
      </div>

      <div className="flex gap-2 bg-primary p-3.5">
        <ShareBtn onClick={() => onShare("insta")} disabled={sharing} className="bg-accent-primary-dark"><Instagram className="size-4" /> Story</ShareBtn>
        <ShareBtn onClick={() => onShare("wa")} disabled={sharing} className="bg-hlhp-good-deep"><MessageCircle className="size-4" /> WhatsApp</ShareBtn>
        <ShareBtn onClick={() => onShare("save")} disabled={sharing} className="bg-accent-secondary-dark"><Download className="size-4" /> Save</ShareBtn>
      </div>
...
function ShareBtn({ children, onClick, disabled, className }: { children: React.ReactNode; onClick: () => void; disabled?: boolean; className?: string }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={cn(
        "flex flex-1 items-center justify-center gap-1.5 rounded-[10px] py-2.5 text-[11px] font-medium text-white disabled:cursor-wait disabled:opacity-70",
        className,
      )}
    >
      {children}
