...
def load(path):
    ...
    # ---- 13. Gender + Life-Stage  (states + State×Concern rules) ----------
    gender_states = []     # [{state, description}]
    gender_rules = {}      # "state|concern" -> {risk_delta, direction, action, addendum, anchor}
    SHEET_GENDER = "13. Gender + Life-Stage"
    if SHEET_GENDER in [ws.title for ws in wb.worksheets]:
        rows = sheet_rows(SHEET_GENDER)
        sec = None
        for r in rows:
            c0 = norm(r[0])
            ...
            if not c0:
                continue
            if sec == "ref":
                # stop the reference section at the next section header
                ...
                gender_rules[f"{slug(c0)}|{slug(concern)}"] = {
                    "state": c0, "concern": concern, "risk_delta": delta,
                    "direction": norm(r[3]), "action": norm(r[4]),
                    "addendum": norm(r[5]), "anchor": norm(r[6]),
                }

    # ---- Time Overlay  (per-band morning anticipatory + evening/night) ----
    # maps factor + band_key -> {morning, evening}. "Band / Key" holds the band
    # LABEL, so we resolve it to the key via the Bands Reference.
    label_to_key = {}
    for fac, bl in bands.items():
        for b in bl:
            label_to_key[(fac, b["label"].lower())] = b["key"]
    time_overlay = {}      # "factor|band_key" -> {morning, evening}
    SHEET_TIME = "Time Overlay"
    if SHEET_TIME in [ws.title for ws in wb.worksheets]:
        started = False
        for r in sheet_rows(SHEET_TIME):
            ...
            if started and fac in ("UV", "AQI", "Temperature", "Humidity"):
                ...
                if not key:
                    continue
                morning = norm(r[3])
                # treat the "— (…)" placeholders as "no morning note"
                if morning.startswith("—"):
                    morning = ""
                time_overlay[f"{slug(fac)}|{key}"] = {
                    "morning": morning, "evening": norm(r[4]),
                }

    return {
        "meta": {
            "source": "SkinBB_HLHP_Scenario_Library_v3_5.xlsx",
            "version": "3.5",
            "note": "Evidence library export — L0/L1/L2 alert text, risk, confidence, PMID anchors, gender/life-stage modifiers, time-of-day overlay. Product-free; 'advice' not used.",
...
if __name__ == "__main__":
    ...
    print(f"  gender states  : {len(data['gender_states'])}")
    print(f"  gender rules   : {len(data['gender_rules'])}")
    print(f"  time overlays  : {len(data['time_overlay'])}")
