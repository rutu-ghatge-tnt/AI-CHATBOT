...
from app.label_looker import ll2_engines
...
def _build_ll2_product(product: dict[str, Any]) -> dict[str, Any]:
...
async def _score_product_impl(*, user: dict[str, Any], body: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.info(
        "LL2 match profile payload userId=%s mode=%s userData=%s",
        user_id,
        mode,
        {
            "age": age,
    ...
    ll2_product = _build_ll2_product(product)
    ll2_product["declared_for_skin_types"] = [x.lower().strip() for x in declared_types if str(x).strip()]
    if isinstance(product.get("keyIngredients"), list):
        ll2_product["key_ingredients"] = await _resolve_ingredients_from_rows(
    ...
    if isinstance(product.get("ingredients"), list):
        ll2_product["ingredients"] = await _resolve_ingredients_from_rows(
            rows=product.get("ingredients"),
            branded_ingredients_coll=branded_ingredient_coll,
            ingredient_coll=ingredient_coll,
        )
    safety = ll2_engines.evaluate_safety(
        age=age if isinstance(age, int) else None,
        life_stages=life_stages,
        conditions=conditions,
        key_ingredients=ll2_product["key_ingredients"],
    )
    logger.info(
        "LL2 product payload productId=%s productData=%s",
        product_ref,
        {
            "name": ll2_product.get("name"),
            "category": ll2_product.get("category"),
            "declaredForTypes": ll2_product.get("declared_for_skin_types"),
            "ingredientsCount": len(ll2_product.get("ingredients") or []),
            "keyIngredientsCount": len(ll2_product.get("key_ingredients") or []),
            "keyIngredientsPreview": [
                row.get("inci_name") for row in (ll2_product.get("key_ingredients") or [])[:10] if isinstance(row, dict)
            ],
        },
    )
    ...
    if state == "gate":
        ...
        suitability = ll2_engines.evaluate_suitability(
    ...
    observations = ll2_engines.evaluate_observations(
        state=state or "low",
        safety=safety,
    ...
    if state == "gate":
        ...
        logger.info(
            "LL2 tile generation payload model=%s user=%s product=%s scoring=%s",
            s.anthropic_model,
    ...
    response: dict[str, Any] = {
        ...
        "cta": cta,
        "full_analysis": {
            "ingredients": ll2_product.get("ingredients", []),
            "key_ingredients": ll2_product.get("key_ingredients", []),
            "claims_checked": ll2_product.get("claims", []),
