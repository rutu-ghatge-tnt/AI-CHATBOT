...
from app.label_looker.core.constants import SOURCE_CLUADE_AI
...
async def _find_ingredient_by_name(coll: AsyncIOMotorCollection, name: str) -> Optional[dict[str, Any]]:
    if not name:
        return None
    if "/" in name:
        ...
        return None
    return await coll.find_one({"name": {"$regex": f"^{escape_regex(name)}$", "$options": "i"}, "isDeleted": {"$ne": True}})
...
async def get_ingredient_detail_for_scanner(*, name: str | None) -> list[dict[str, Any]]:
    ...
    existing = await art_coll.find_one({"status": "approved", "$or": [{"ingredientId": ing_id}, {"ingredientId": parent_id}]})
    if not existing:
        ...
        await _create_article_from_ai(article_coll=art_coll, user_coll=user_coll, ingredient_id=parent_id if isinstance(parent_id, ObjectId) else ing_id, payload=parsed)
