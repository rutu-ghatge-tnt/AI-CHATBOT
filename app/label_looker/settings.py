...
def get_label_looker_settings() -> LabelLookerSettings:
    ...
    if not base:
        raise RuntimeError(
            "SKIN_BB_BASE_URL (or CREDITS_API_BASE_URL/SERVER_URL fallback) is required for Label Looker routes"
        )
    ...
    return LabelLookerSettings(
        ...
        include_error_stack=os.getenv("LABEL_LOOKER_INCLUDE_STACK", "true").lower()
        in ("1", "true", "yes"),
    )
